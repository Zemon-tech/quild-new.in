        ...
        <CarouselContent className="-ml-4">
          {item.images.map((img, idx) => (
            <CarouselItem
              ...
              className={cn(
                "pl-4",
                isMobile ? "basis-full" : "basis-[75%]"
              )}
            >
              <div style={{
                width: '100%',
                aspectRatio: isMobile ? '4/3' : '21/10',
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontFamily: 'var(--font-dm-sans)', fontSize: '0.9rem', color: 'var(--ink)' }}>
              <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: status.dot, animation: item.status === 'active' ? 'pulse-dot 2s ease-in-out infinite' : 'none', flexShrink: 0 }} />
              {status.label}
            </div>
          </div>
        </div>
    ...
    <section style={{ background: 'var(--bg)', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row' }}>
        ...
        {isMobile ? (
          ...
          <aside style={{
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                      {statusMeta && (
                        <div style={{
                          ...
                          flexShrink: 0,
                          animation: filter === 'HAPPENING NOW'
                    ...
                    <span style={{
                      ...
                      color: isActive ? 'var(--sage)' : 'var(--border)',
                      transition: 'color 0.2s',
                    }}>
                      {filter === 'ALL'
                        ? INITIATIVES.length
                        : INITIATIVES.filter(i => i.status === FILTER_TO_STATUS[filter]).length}
                    </span>
                  </button>
                )
              })}
            ...
            <div style={{
              ...
              fontFamily: 'var(--font-jetbrains-mono)',
              fontSize: '0.52rem', color: 'var(--border)',
              letterSpacing: '0.12em', textTransform: 'uppercase',
              lineHeight: 1.6,
            }}>
              NON-PROFIT ·<br />ALL FIELDS WELCOME
            </div>
          </aside>
        )}
        ...
        >
          {filtered.length > 0 ? (
            filtered.map(item => (
              <InitiativeRow key={item.id} item={item} isMobile={isMobile} />
            ))
          ) : (
            <div style={{
              padding: '4rem',

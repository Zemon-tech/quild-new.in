...
const CAROUSEL_SCREENS = [
  { image: "/footer.png", title: "CAPTURING MOMENTS,\nCREATING MEMORIES" },
  { image: "/careers.png", title: "BUILDING THE NEXT\nGREAT GENERATION" },
  { image: "/about.png", title: "FINDING THE BUILDERS\nWHO STAND OUT" },
];
        ...
        <div className="hidden lg:flex w-full lg:w-[69%] relative overflow-hidden bg-[var(--void)] flex-col justify-between p-10 min-h-[750px] transition-all border-r border-white/10">
          <div className="absolute inset-0 z-0">
            {CAROUSEL_SCREENS.map((screen, idx) => (
              <div key={idx} className="absolute inset-0 transition-opacity duration-1000 ease-in-out" style={{ opacity: activeScreenIndex === idx ? 1 : 0 }}>
                <Image src={screen.image} fill className="object-cover" priority={idx === 0} alt={`Slide ${idx + 1}`} />
              </div>
            ))}
          </div>
          ...
          <div className="relative z-20 flex flex-col gap-8 pb-4">
            <div className="relative min-h-[140px]">
              {CAROUSEL_SCREENS.map((screen, idx) => (
                <div key={idx} className="absolute bottom-0 left-0 w-full transition-all duration-1000 ease-in-out"
                  style={{ opacity: activeScreenIndex === idx ? 1 : 0, transform: `translateY(${activeScreenIndex === idx ? '0px' : '15px'})`, pointerEvents: activeScreenIndex === idx ? 'auto' : 'none' }}>
                  <div className="font-mono text-[0.7rem] uppercase tracking-[0.15em] text-white/50 mb-6">0{idx + 1} / 03 — IN FOCUS</div>
                  <h1 className="font-display font-semibold leading-[0.9] tracking-[-0.02em] text-white whitespace-pre-line"
            ...
            <div className="flex gap-3 items-center mt-2 border-t border-white/20 pt-6">
              {CAROUSEL_SCREENS.map((_, idx) => (
                <button key={idx} onClick={() => setActiveScreenIndex(idx)}
                  className="h-[2px] transition-all duration-500 ease-in-out cursor-pointer"
                  style={{ width: activeScreenIndex === idx ? "40px" : "20px", backgroundColor: activeScreenIndex === idx ? "#FFFFFF" : "rgba(255,255,255,0.2)" }}
          ...
          <div className="w-full max-w-[420px] mx-auto space-y-10">
            {submitted ? (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full justify-center min-h-[50vh]">
                <div className="mb-8 font-mono text-[0.7rem] uppercase tracking-[0.12em] text-[var(--sage)]">SYSTEM / STATUS: 200 OK</div>
                <h2 className="font-display text-[clamp(2.5rem,5vw,3.5rem)] font-semibold leading-[1] text-[var(--ink)] mb-6">Access<br />Granted.</h2>
                ...
                <Link href="/"><Button className="w-full rounded-none bg-[var(--ink)] text-[var(--bg)] hover:bg-[var(--ink)]/90 h-[52px] font-mono text-[0.75rem] uppercase tracking-[0.1em] transition-all">ENTER MAINFRAME →</Button></Link>
              </div>
            ) : (
              <div className="animate-in fade-in duration-500">
                <div className="mb-10">
                  <div className="font-mono text-[0.65rem] uppercase tracking-[0.12em] text-[var(--muted)] mb-4 h-[12px]">{!isLogin && `STEP 0${step} / 03`}</div>
                  ...
                  <p className="font-mono text-[0.7rem] uppercase tracking-[0.05em] text-[var(--muted)] leading-relaxed border-l border-[var(--border)] pl-3">
                    ...
                    ) : step === 2 ? "TRANSMIT DIRECTIVES ALONG WITH BUILDER PROFILE" : "VERIFY SYNC DATA. SUBMIT TO NETWORK."}
                  </p>
                </div>

                {errorMsg && (
                  <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 text-red-600 font-sans text-sm rounded-none">{errorMsg}</div>
                )}
                    ...
                    {isLogin ? (
                      <form onSubmit={handleLogin} className="space-y-5">
                        ...
                        <div className="space-y-1.5 relative w-full">
                          ...
                          <Input name="password" value={formData.password} onChange={handleChange} required type={showPassword ? "text" : "password"} placeholder="********"
                          ...
                          <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-[28px] text-[var(--muted)] hover:text-[var(--ink)] transition-colors">
                      ...
                      <form onSubmit={(e) => { e.preventDefault(); setStep(2); setErrorMsg(""); }} className="space-y-5">
                        ...
                        <div className="flex items-center space-x-3 pt-2 pb-2">
                          ...
                          <label htmlFor="terms" className="font-mono text-[0.65rem] tracking-[0.05em] uppercase text-[var(--muted)] leading-none cursor-pointer flex items-center gap-1 mt-[2px]">
                            Accept
                            <Dialog>
                              <DialogTrigger className="text-[var(--ink)] underline decoration-[var(--border)] hover:decoration-[var(--ink)] underline-offset-4 transition-colors">Policy & Rules</DialogTrigger>
                              <DialogContent className="border border-[var(--border)] text-[var(--ink)] sm:max-w-[425px] overflow-hidden rounded-none shadow-2xl">
                                <DialogHeader>
                                  <DialogTitle className="font-display tracking-[0.05em] text-xl">NETWORK POLICIES</DialogTitle>
                                  <DialogDescription className="font-mono text-[0.7rem] uppercase tracking-[0.05em] text-[var(--muted)] mt-2">[ STRICT GOVERNANCE PROTOCOL ACTIVE ]</DialogDescription>
                                </DialogHeader>
                                <div className="font-sans text-[0.95rem] leading-[1.6] text-[var(--muted)] space-y-6 mt-4 max-h-[50vh] overflow-y-auto pr-4">
                                  <p><strong className="text-[var(--ink)] font-mono text-[0.7rem] uppercase tracking-[0.1em] block mb-1">01 / ACCEPTANCE</strong>By bridging to this network, you submit to all parameters formulated within.</p>
                                  <p><strong className="text-[var(--ink)] font-mono text-[0.7rem] uppercase tracking-[0.1em] block mb-1">02 / DATA PROTOCOL</strong>Inputs drop firmly into our encrypted stores. No leaks. No telemetry swaps without dual-consent.</p>
                                  <p><strong className="text-[var(--ink)] font-mono text-[0.7rem] uppercase tracking-[0.1em] block mb-1">03 / CONDUCT</strong>Build, do not destroy. Noise generators will be forcefully disconnected.</p>
                                </div>
                              </DialogContent>
                            </Dialog>
                          </label>
                        </div>
                        <Button type="submit" className="w-full rounded-none bg-[var(--ink)] hover:bg-[var(--ink)]/90 text-[var(--bg)] h-[48px] font-mono text-[0.7rem] uppercase tracking-[0.1em] transition-all">CONTINUE (1/3) →</Button>
                      </form>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <Button type="button" variant="outline" className="w-full text-[var(--ink)] bg-transparent border-[var(--border)] hover:bg-[var(--ink)]/5 rounded-none h-[44px] transition-all font-mono text-[0.65rem] uppercase tracking-[0.1em]">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-[15px] h-[15px] mr-1 shrink-0">
                          <path d="M16.3653 10.1558C16.3862 7.42555 18.6083 6.11054 18.7067 6.04944C17.4475 4.20573 15.5188 3.92135 14.8687 3.84078C13.1834 3.66812 11.5363 4.8364 10.6865 4.8364C9.84307 4.8364 8.44185 3.85694 7.03437 3.88567C5.23253 3.91136 3.56515 4.93988 2.63737 6.55928C0.751325 9.85196 2.15509 14.7176 3.99308 17.3828C4.89311 18.6791 5.95254 20.1416 7.33306 20.086C8.67814 20.0267 9.18331 19.2059 10.8066 19.2059C12.4282 19.2059 12.8943 20.086 14.2829 20.0573C15.7118 20.0267 16.6215 18.7366 17.5144 17.4367C18.5492 15.9143 18.9804 14.4368 19 14.3649C18.9669 14.3512 16.3429 13.33 16.3653 10.1558ZM13.8837 2.45493C14.6309 1.54947 15.127 0.288599 14.9905 -0.966309C13.914 0.0768469 12.6033 0.750133 11.823 1.68414C11.1341 2.50285 10.5362 3.79152 10.7042 5.01357C11.9056 5.10541 13.1203 4.38072 13.8837 2.45493Z" />
                        </svg>
                ...
                {step === 2 && (
                  <form onSubmit={(e) => { e.preventDefault(); setStep(3); setErrorMsg(""); }} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-8 pt-4">
                      <Button type="button" variant="outline" className="rounded-none bg-transparent border-[var(--border)] text-[var(--ink)] hover:bg-[var(--ink)]/5 h-[48px] transition-all font-mono text-[0.7rem] uppercase tracking-[0.1em] px-0" onClick={() => setStep(1)}>&larr; REVERT</Button>
                      <Button type="submit" className="rounded-none bg-[var(--ink)] hover:bg-[var(--ink)]/90 text-[var(--bg)] h-[48px] font-mono text-[0.7rem] uppercase tracking-[0.1em] transition-all px-0">CONTINUE (2/3) &rarr;</Button>
                    </div>
                  </form>
                )}
                ...
                {step === 3 && (
                  <div className="space-y-8">
                    <div className="border border-[var(--border)] bg-[var(--bg)] p-6 border-l-2 border-l-[var(--sage)]">
                      ...
                      <p className="text-[0.95rem] leading-[1.65] text-[var(--muted)] font-sans">You are about to submit your application profile to the Quild network. Review cautiously. Once committed, the registry is sealed.</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <Button type="button" variant="outline" className="rounded-none bg-transparent border-[var(--border)] text-[var(--ink)] hover:bg-[var(--ink)]/5 h-[48px] transition-all font-mono text-[0.7rem] uppercase tracking-[0.1em] px-0" onClick={() => setStep(2)}>&larr; REVERT</Button>
